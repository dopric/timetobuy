    ...
    public class SeedData
    {
        public static void Initialize(IServiceProvider serviceProvider)
        {
            using(var context = new DatabaseContext(serviceProvider.GetRequiredService<DbContextOptions<DatabaseContext>>()))
            {
                ...
                {
                    InitilizeProducts(context);
                }
            }
        }

        static void InitilizeProducts(DatabaseContext context)
        {
            context.Products
                ...
                { Name = "Learning React", Description = "some description", Price = 29M, Image = "" },
                          new Product
                { Name = "React advanced", Description = "ready to take next step?", Price = 35.5M, Image = "" },
                          new Product
                { Name = "Asp.Net Core & React", Description = "Learn C#, WebApi and React", Price = 21.7M, Image = "" },
                          new Product
                { Name = "React & Redux", Description = "React for enterprise applications", Price = 30M, Image = "" },
                          new Product
                {
                    Name = "SPA with React",
